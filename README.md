...
* `listings.html`: The main marketplace page where buyers browse all listings, later replaced with `src/dashboard-buyer.php` to connect with the database to backend.
* `scrap-detail.html`: The detailed view for a single scrap item, later replaced with `src/details.php` to connect with the database to backend.
* `seller.html`: The seller-facing dashboard for managing listings and sales, later replaced with `src/dashboard-seller.php` to connect with the database to backend.
